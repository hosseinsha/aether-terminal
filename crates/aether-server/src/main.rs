...
//!     attached client.
//!
//! A single client connection can attach to many sessions at once (the app's
//! panes). The same protocol runs over a Unix socket today and over TCP+TLS for
//! remote hosts tomorrow — only the listener changes.
...
async fn main() -> Result<()> {
    ...
    eprintln!("aether-server listening on {}", path.display());
    ...
    loop {
        ...
        match msg {
            ...
            ClientMsg::Attach { id } => {
                ...
                // Snapshot first, then subscribe — both under no race because the
                // grid mirror and the broadcast are fed by the same byte stream.
